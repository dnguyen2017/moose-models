...
```{r no_harvest, out.width = "50%", fig.show = 'hold'}
plist[[1]] + 
  theme(legend.position = "n") +
  labs(title = paste("Density-dependent tick attachment", 0, "% annual harvest"))
  

filter(df_flat, near(harvest, 0) & t < 50 & stage %in% c("cows","calves") ) %>%
      ggplot(mapping = aes(x = t, y = abundance_or_mean, fill = stage)) +
      geom_area(colour = "white") +
      labs(title = paste("Density-independent tick attachment", 0, "% annual harvest"),
           fill = "moose stage/ \ntick source",
           x = "time (years)",
           y = "abundance of moose or questing larvae") +
      theme_bw() + 
...
```{r coexistence, out.width = '50%', fig.show = 'hold'}
...
```{r controlled, out.width = '50%', fig.show = 'hold'}
